...
public class Classifier {
	...
	public Classifier() throws Exception {
		...
		ConverterUtils.DataSource source = new ConverterUtils.DataSource("weka/trainingModel1.arff");
		...
		source = new ConverterUtils.DataSource("weka/trainingModel2.arff");
		...
		source = new ConverterUtils.DataSource("weka/trainingModel3.arff");
	...
	public void reTrain(List<List<VehicleData>> vehicleModel, int dayPeriod) throws Exception {
		dataSetManager.createTrainingData(vehicleModel);

		ConverterUtils.DataSource source;

		switch (dayPeriod) {
		case 1:
			source = new ConverterUtils.DataSource("weka/trainingModel1.arff");
			trainInstances1 = source.getDataSet();
			trainInstances1.setClassIndex(trainInstances1.numAttributes() - 1);
			classifierFirstPart.buildClassifier(trainInstances1);
			break;
